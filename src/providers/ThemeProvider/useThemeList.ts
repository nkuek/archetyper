...
const useThemeList = () => {
  ...
  return useMemo<IThemeList>(
    () => ({
      ...
      default: {
        buttonBackground: isDarkTheme ? 'black' : 'white',
        pageBackground: isDarkTheme ? 'black' : 'white',
        wordBoxBackground: isDarkTheme ? '#333' : 'white',
        currentWord: 'plum',
        currentChar: 'darkmagenta',
        words: isDarkTheme ? 'white' : 'black',
        correct: 'green',
        headings: isDarkTheme ? 'white' : 'black',
        border: `1px solid ${isDarkTheme ? 'white' : 'black'}`,
        disabled: '#ddd',
